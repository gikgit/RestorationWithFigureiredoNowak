...
float** LoadImagePgm(char* name,int *length,int *width)
{
  ...
  fic=fopen(buff,"r");
  if (fic==NULL)
    { printf("\n- There is en error happened when open %s  -\n",buff);
      exit(-1); }
...
void SaveImagePgm(char* name,float** mat,int length,int width)
{
  ...
  fic=fopen(buff,"w");
    if (fic==NULL) 
        { printf(" Problem when save %s",buff); 
          exit(-1); }
...
int diff(float**haar_inverse,float**f,int length, int width)
{
	int i,j;
	float sum1=0.0,sum2=0.0;
	for(i=0;i<length;i++){
		for(j=0;j<width;j++){
			sum1=sum1+SQUARE(haar_inverse[i][j]-f[i][j]);	
			sum2=sum2+SQUARE(f[i][j]);	
		}	
	}
	if((sum1/sum2)>pow(10,-5)){
		return 0;	
	}
	else{
		return 1;
